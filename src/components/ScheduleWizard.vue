...
function canProceed() {
  ...
  if (currentStep.value === 1) {
    if (!formData.value.flowId) {
      errors.value.flowId = 'Please select a flow'
      return false
    }
  }
...
const selectedFlow = computed(() => {
  return props.availableFlows.find(f => f._id === formData.value.flowId)
})
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="currentStep === 1" class="space-y-4">
      <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Select Flow</h3>
        <p class="text-sm text-gray-600 mb-4">Choose the flow you want to schedule</p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3">
        <button
          ...
          :key="flow._id"
          @click="formData.flowId = flow._id"
          :class="[
            'p-4 rounded-lg border-2 text-left transition-all',
            formData.flowId === flow._id
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <div class="text-sm text-gray-600 mt-1">{{ flow.description || 'No description' }}</div>
            </div>
            <Icon
              v-if="formData.flowId === flow._id"
      ...
      <p v-if="errors.flowId" class="text-red-600 text-sm">{{ errors.flowId }}</p>
    </div>

    <!-- Step 2: Configure Schedule -->
    <div v-if="currentStep === 2" class="space-y-6">
      <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Configure Schedule</h3>
        <p class="text-sm text-gray-600 mb-4">Define when and how often this flow should run</p>
      </div>
      ...
      <div class="space-y-4">
        <div v-for="input in flowInputs" :key="input.name" class="space-y-2">
          <label class="block">
            ...
            <p v-if="input.description && input.type !== 'boolean' && !isDateField(input)" class="text-xs text-gray-500 mt-1">
              {{ input.description }}
            </p>
